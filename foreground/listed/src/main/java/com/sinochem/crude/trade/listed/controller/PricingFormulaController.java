...
import com.sinochem.crude.trade.listed.enums.PriceFormulaType;
import com.sinochem.crude.trade.listed.model.form.PricingFormulaInfoForm;
...
public class PricingFormulaController {
	 ...
	 * @return
	 */
	@RequestMapping(value = UrlMapping.PRICINGFORMULA_PHYSICALPRICINGEDITOR)
    public String priceEditor(ModelMap modelMap){
    	...
    	return "crudeoilhallform/physicalPricingEditor";
    }
	 ...
	 */
    @ResponseBody
	@RequestMapping(value = UrlMapping.PRICINGFORMULA_GETPRICINGINFO)
	public ResultDatas<PricingFormulaInfoForm> getPricingInfo(PricingFormulaInfoForm info) {
		ResultDatas<PricingFormulaInfoForm> res = new ResultDatas<PricingFormulaInfoForm>();
		try{
			processFormula(info);
			res.setDatas(info);
			return res;
			...
			res.setMessage(VisitorLocale.getByCurrentLanguage(MsgConstant.LISTED0083));
			return res;
		}
	}
     ...
     * @param info
     */
    private void processFormula(PricingFormulaInfoForm info){
...
//    	PriceFormulaType formulaType = PriceFormulaType.getFormulaTypeByCode(info.getFormulaType());
    	...
    	}else if(PriceFormulaType.Trigger.getFormulaType().equalsIgnoreCase(formulaType)){//点价
			processTriggerFormula(info);
     ...
     * @param info
     */
    private void processFixedFormula(PricingFormulaInfoForm info){
     ...
     * @param info
     */
    private void processAverageFormula(PricingFormulaInfoForm info){
	 ...
	 * @param info
	 */
	private void processTriggerFormula(PricingFormulaInfoForm info){
		Map<String,Object> quoteMp = new LinkedHashMap<String,Object>();//一个市场合约
		quoteMp.put("commodity", info.getCommodity());//报价商品
		quoteMp.put("price_source", info.getPriceSource());//来源
		quoteMp.put("market", info.getMarket());//市场
		...
		detailMp.put("uom", info.getUom());
		detailMp.put("currency", info.getCurrency());
		detailMp.put("monthOuterationTime", info.getMonthOuterationTime());
		detailMp.put("minOperAbleNum", info.getMinOperAbleNum());
		SimpleDateFormat dateFormat = new SimpleDateFormat("MMM/dd/yyyy",Locale.ENGLISH);
//		detailMp.put("date1", dateFormat.format(info.getStartDate()));//"Jan/01/2018", //计价期起
//		detailMp.put("date2", dateFormat.format(info.getEndDate()));//"Jan/31/2018"//计价期止
		detailMp.put("deadline", dateFormat.format(info.getDeadline()));//"Jan/31/2018"//点价截止时间

		Map<String,Object> formudMp = new HashMap<String,Object>();
		formudMp.put("trigger_pricing_detail", detailMp);
		...
		formula.append(" Roll over option:").append(info.getMonthOuterationTime());
		formula.append(" Min operational Qty:").append(info.getMinOperAbleNum());

		SimpleDateFormat dateFormat2 = new SimpleDateFormat("yyyy-MM-dd");
		formula.append(" Trigger deadline:").append(dateFormat2.format(info.getDeadline()));
     ...
     * @param info
     */
    private void processEventFormula(PricingFormulaInfoForm info){
    	Map<String,Object> quoteMp = new LinkedHashMap<String,Object>();//一个市场合约
    	quoteMp.put("commodity", info.getCommodity());//报价商品
    	quoteMp.put("price_source", info.getPriceSource());//来源
    	quoteMp.put("market", info.getMarket());//市场
