$(function() {
	...
	$(window).resize(function() {
		var body_with = parseInt($('body').css('width'));
		var left_menu_width = parseInt($('.s-side').css('width'));
		$('.s-body').animate({  'width': (body_with-left_menu_width) +'px' }, 500);	 
	}); 
	function toggle() {
	
		sildeWidth = 50;//缩小侧边栏宽度
		picWidth_x = -(parseInt($('.s-side').css('width'))-sildeWidth);//margin-left
		var body_width = parseInt($('.s-body').css('width'));
		user_pic = 40
	    if (parseInt($('.s-side').css('width'))>100) {
			...
			$('.s-body').animate({ 'margin': '0px 0px 0px '+picWidth_x+'px', 'width': body_width-picWidth_x +'px' }, 600);//content
			...
			$('.s-body').animate({ 'margin': '0px 0px 0px 0px ', 'width': body_width-200 +'px'}, 400);//content
