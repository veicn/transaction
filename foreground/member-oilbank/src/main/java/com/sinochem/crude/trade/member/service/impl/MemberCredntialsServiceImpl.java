...
//                    roleCodeList.add(credentialsDetail.getRole());
    ...
    public void updateUdbCredentials(String[] types, Long memberId) {
        if(types == null || memberId == null || types.length==0) {
            logger.info("没有需要更新的资质");
            return;
        }
    ...
    @Override
    public MemberCredentials queryEnterpriseCredential(Long id, String code) {
        return memberCredentialsMapper.queryEnterpriseCredential( id, code);
    }
