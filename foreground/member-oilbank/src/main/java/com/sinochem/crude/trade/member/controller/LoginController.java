...
public class LoginController {
	...
	public ModelAndView loginReturnUrl(HttpSession httpSession, String returnUrl,Integer reg,Integer rego, HttpServletRequest request, HttpServletResponse response) {
...
////		}
       ...
       // List<String> userRoles = memberCredentialsService.getByMember(user.getMemberId());
		...
		if (user.isPlatformUser()) {
			...
			buildReturn(returnUrl,this.appServerBroker.get("om/welcome.htm").toString(),request, null, modelAndView,response );
			return modelAndView;
		}
...
//		}
        ...
        if(user.getMemberId()==100008253||user.getMemberId()==100008254){
            String[] strArray={"sales_trader","buy_trader","trade_executor","trade_executor","inspection","ship_owner"
                    ,"ship_broker","depa_port","arriv_port","ship_trader","ship_executor","enter_admin"
                    ,"enter_master","charterer","platform_charterer","admin","enterprise","info_oper"};
            superUser.setRoles( strArray );
        ...
        if(null!=returnUrl&&returnUrl.contains("logincall")&&null!=userInfo.get("epMemberId")) {
            returnUrl = returnUrl + "?_epId=" + String.valueOf(userInfo.get("epMemberId")) + "&_mid=" + String.valueOf(userInfo.get("memberId"));
            Cookie cookie = new Cookie("_theme", "block");
            cookie.setDomain(domain);
            cookie.setPath("/");
            response.addCookie(cookie);
            Cookie cookie1 = new Cookie("_l_", "zh_CN");
		...
		buildReturn(returnUrl,target, request, result, modelAndView,response);
		return modelAndView;
	}

	private void buildReturn(String returnUrl,String targetUrl, HttpServletRequest request, ResultDatas result ,ModelAndView modelAndView,HttpServletResponse response) {
