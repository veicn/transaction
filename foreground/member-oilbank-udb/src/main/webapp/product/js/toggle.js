...
$(function(){
	...
	function toggle() {
	    if ($('.c-ml').css('width') == '250px') {
	        ...
	        $('.c-header .c-logo').css({ 'backgroundPosition': '-40px center' });
	        $('.c-header .c-logo').animate({ 'width': '50px' }, 600);
	        $('.c-nav .li-search').css({ 'position':'relative' });
	        $('.c-nav .li-search').animate({'left': '-200px' }, 600,function(){

                $('.ico-arrr').addClass("ico-arrl").removeClass("ico-arrr");
			});
        } else {
	        toggleyes = true;
	        $('.c-menu>li>a').attr('moren', 'true');
	        ...
	        $('.c-company p').delay(400).animate({ 'height': '40px' }, 0);
            ...
            $('.c-header .c-logo').css({ 'backgroundPosition': 'center center' });
	        $('.c-header .c-logo').animate({ 'width': '250px' }, 600);
	        $('.c-nav .li-search').animate({ 'position':'relative','left': '0px' }, 600,function(){
                $('.ico-arrl').addClass("ico-arrr").removeClass("ico-arrl");
			});
	        $('.c-menu>li>ul').hide();
	    }
	}
