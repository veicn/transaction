 ...
 */
@Controller
@RequestMapping(UrlMapping.EDIT_DEMAND_SHEET)
public class EditDemandSheetController {
     ...
     * 修改商品采购资源
     */
    @RequestMapping(value=UrlMapping.COMMIT_DEMAND_SHEET_EDIT)
    @ResponseBody
    public Result editProduct(DemandSheetCombineVO demandSheetCombineVO, CurrentUser currentUser) {

        //ModelAndView mav = new ModelAndView();
        Result res = new Result();
        try {
            DemandSheetCombine demandSheetCombine = demandSheetCombineVO.getDomain();
            demandSheetService.updateDemandSheetCombineSaveHistory(currentUser, demandSheetCombine);

            res.setStatus(Mark.RESULT_DATA_SUCCESS);
            res.setMessage(Mark.EDIT_SUCCESSFULLY);
            //mav.setViewName("redirect:/" + UrlMapping.MY_PRODUCT_LIST + "/" + UrlMapping.INDEX + ".htm?uuid=" + demandSheetCombine.getSaleSheet().getUuid());
        } catch (BizException e) {
            e.printStackTrace();
            int exceptionCode = e.getCode();
