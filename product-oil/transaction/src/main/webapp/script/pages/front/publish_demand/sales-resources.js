(function () {
    ...
    BasicInfo.prototype.initRadios = function(){
        var _self = this;
        commonCPY.radios(function(obj,e){
    ...
    BasicInfo.prototype.deleteTr = function(){
        ...
        $('.inquiry-private-table').on('click','tbody .delete',function(){
            ...
            var ind=$('.inquiry-private-table .delete').index($(this));
    ...
    BasicInfo.prototype.createQuoter = function () {
        ...
        layui.use('layer', function(){
            ...
            this.indexCompany = layer.open({
                ...
                '<button class="on-shelf fr">Add</button>'+
                '</div>',
                success:function(){
    ...
    BasicInfo.prototype.addEnterpriseList = function () {
        var _self = this;
        // var arr=[];
        $(document).on('click','.on-shelf',function () {
            $('.inquiry-private-table tbody').remove();
            var count = $('.inquiry-private-table tbody').children('tr').length;
            var imageUrl = appServer + '/images/delete-icon.png';
           if(_self.table==null){
               return;
           }
            var enterpriseList = _self.table.checkStatus('Company').data;
            for (var i = 0; i < enterpriseList.length; i++) {
                var obj={}
                var enterprise = enterpriseList[i];
                var name = enterprise.name;
                var memberId = enterprise.memberId;
                obj.name=name;
                obj.memberId=memberId;
                arr.push(obj);
                var hash = {};
                arr = arr.reduce(function(item, next) {
                    hash[next.memberId] ? '' : hash[next.memberId] = true && item.push(next);
                    return item
                }, [])
    ...
    BasicInfo.prototype.fuzzyQuery = function (keyWord) {
        ...
        if(!keyWord){
            //如果输入框是空的情况
            return arr = this.listTableData;
        }
        var reg = new RegExp(keyWord.toString().toLowerCase(),"i");
        var name;
        for(var i=0;i<len;i++){
            name = !!this.listTableData[i].name?this.listTableData[i].name:''
            if(name.match(reg)){
    ...
    BasicInfo.prototype.tableSearch = function () {
        ...
        $(document).on('keyup','.search-wrap input',this.throttle(function () {
            obj = _self.fuzzyQuery($(this).val().toString().toLowerCase());
            _self.tableRender(obj);
        },300));
    };
