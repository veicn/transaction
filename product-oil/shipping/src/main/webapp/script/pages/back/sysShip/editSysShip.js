...
$(function() {
	 commonCPY.select();
	// 提交form表单
	$('#editSysShipForm2').click(function() {
        ...
        var arr=$('#editSysShipForm');
        commonCPX.FormValidation(arr);
        if(!commonCPX.FormValidation(arr)) {
            $.ajax({
                type: "POST",
                url: '/shipping/pages/back/sysShip/updateSysShip.json',
                data: $("#editSysShipForm").serialize(),// 序列化表单提交input值
...
/*$(function(){
    $("#br btn-b Reset").click(function(){
        $(':input','#saveSysShipForm')
