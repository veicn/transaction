<template>
  <div class="Interface-cont2">
    ...
    <table class="dataintable">
      ...
      <tr v-if="datas" v-for="(v,index) in datas " :key='index' class="h50">
        <td>{{v.date}}
        </td>
        <td>done</td>
        <td>
          <div style="width:186px">
            ...
            <span class="c1-btn" @click="delet(v.uuid)">Delete</span>
          </div>
        </td>
      </tr>

      <!-- 增加选择框 -->
      <tr class="h50"  v-for="(item,index) in arr " :key='index'>
        <td>
          {{item.DATE}}
        </td>
        <td>null</td>
        <td>
          <div style="width:186px">    
            ...
            <span class="c1-btn"  @click="remove(index)">Delete</span>
          </div>
        </td>
      </tr>
    </table>
    <mt-button class="c1btn" @click="addToTd()">＋Add</mt-button>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    addToTd:function(){
     this.arr.push({DATE: new Date().format('yyyy MM dd')})
    }
  },
  mounted() {}
};
