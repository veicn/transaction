layui.use(['layer', 'form', 'jquery','laypage', 'table', 'laydate','own','upload'], function(){
	...
	function message(mess){
  	...
  	$('.addShip').on('click',function(){
  		window.location.href=shipServer+"/shipPlate/addSysShip.htm";
  	});
					...
					if(arr!=null){
						...
						$.ajax({
				    		...
				    		success: function(data2) {
								if(data2.status == 0){
									...
									currentUserName=data2.datas.epMemberName;	
									currentUserEpId=data2.datas.epMemberId;	
						...
						if(!uuid){
							...
							if(isContains(currentUserRoles,"ship_owner")){		//为船东
								...
								$("#owner").attr("nameFn",currentUserName);
								$("#owner").attr("code",currentUserEpId);
							}
							if(isContains(currentUserRoles,"ship_broker")){		//为经纪人
								//经纪人显示当前用户名
								$("#broker").val(currentUserName);
								$("#broker").attr("nameFn",currentUserName);
								$("#broker").attr("code",currentUserEpId);
							}
						}
    		...
    		success: function(data) {
				if(data.status == 0){
					...
					if(data.datas.status=='5'){		//若船盘的状态为 5未发布，发布日期显示未发布
						$("#date4").val($.i18n("JAVASCRIPT00154"));
					}else{
					...
					var paramsShip = {};
				 	...
				 	$("#name").val(data.datas.name);					/**船名赋初始值*/
				 	$("#name").attr('code',data.datas.sysShipUuid);		/**船uuid赋初始值*/
					...
					$("#owner").val(data.datas.shipOwner);				/**船东*/
					$("#owner").attr('code',data.datas.shipOwnerId);	/**船东uuid*/
					$("#broker").val(data.datas.brokerName);			/**经纪人*/
					$("#broker").attr('code',data.datas.brokerId);		/**经纪人uuid*/
	...
	$('.saveBtn').on('click', function() {
		...
		// 船名校验
		if($("#name").val()=="" || $("#name").val()==null || $("#name").attr("code")=="" || $("#name").attr("code")==null) {
		...
		//校验船东
		if($("#owner").val()=="" || $("#owner").val()==null || $("#owner").attr("code")=="" || $("#owner").attr("code")==null) {
				...
				success: function(data) {
						if(data.status == 0){
							layer.alert($.i18n("CONSTANTS017"),{title:$.i18n("001"),btn:$.i18n("002")}, function(index) {
							    layer.close(index);
							    window.location.href=shipServer+"/shipPlate/shipPlateList.htm";
							});
						} else {
							layer.alert($.i18n(data.message),{title:$.i18n("001"),btn:$.i18n("002")}, function(index) {
							    layer.close(index);
