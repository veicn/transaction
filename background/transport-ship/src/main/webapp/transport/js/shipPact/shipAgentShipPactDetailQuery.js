layui.use(['layer', 'form', 'jquery','laypage', 'table', 'laydate','own'], function(){
	...
	var intentionUuid = own.getHref('intentionUuid');//询盘单UUID
	
	if (intentionUuid){
		var obj ={}; 
		obj.shipIntentionUuid=intentionUuid;
		searchByIntentionOrPlateUuid(obj);
	}
	var plateUuid = own.getHref('plateUuid');//船盘UUID
	if (plateUuid){
		var obj ={}; 
		obj.shipPlateUuid=plateUuid;
		searchByIntentionOrPlateUuid(obj);
	}
	//查询经纪人和船东和船名
	if (uuid){ 	edit(uuid) 	}
	function searchByIntentionOrPlateUuid(obj){
		
		$.ajax({
    		type: "POST",
    		url: shipServer+"/shipPact/findShipPactDetailIntentionOrPlate.json",
