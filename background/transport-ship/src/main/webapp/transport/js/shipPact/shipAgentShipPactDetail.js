layui.use(['layer', 'form', 'jquery','laypage', 'table', 'laydate','own'], function(){
		...
		$.ajax({
    		...
    		success: function(data) {
				if(data.status == 0){
					...
					layer.alert(data.message,  {title:$.i18n("001"),btn:$.i18n("002")}, function(index) {layer.close(index);});
				}
			},
			error: function(data) {
    		...
    		success: function(data) {
				if(data.status == 0){
					if(uuid){
						...
						window.location.href=shipServer+"/shipPact/shipAgentShipPactList.htm";
						});
	...
	function check(obj){
		var flag = true;
		if (obj.signDates == null || obj.signDates == ''){message('请选择租约日');flag=false;return false;}
		if (obj.carrierName == null || obj.carrierName == ''){message('请填写租船人');flag=false;return false;}
		if (obj.pactBegs == null || obj.pactBegs == ''||obj.pactEnds == null || obj.pactEnds == '' ){message('请填写装期信息');flag=false;return false;}
		if (obj.minQuantity == null || obj.minQuantity == ''){message('请填写最小货量');flag=false;return false;}
		if (obj.shipName == null || obj.shipName == '' || obj.sysShipUuid == ''){message('请选择船名');flag=false;return false;}
		if (obj.demurrage == null || obj.demurrage == ''){message('请填写滞期费率');flag=false;return false;}
		if (obj.shipOwner == null || obj.shipOwner == '' || (obj.shipOwner == '其它' && (obj.shipOwners == null ||obj.shipOwners ==''))){message('请选择船东');flag=false;return false;}
		if (obj.pactSpeed == null || obj.pactSpeed == ''){message('请填写合同航速');flag=false;return false;}
		if (obj.brokerName == null || obj.brokerName == '' || (obj.brokerName == '其它' && (obj.brokerNames == null || obj.brokerNames == ''))){message('请选择经纪人');flag=false;return false;}
		if (obj.dockTime == null || obj.dockTime == ''){ message('请填写允许装卸时间');flag=false;return false;}
		if (obj.shipPlateUuid == null || obj.shipPlateUuid == '' || !obj.shipPlateUuid ){message('船盘uuid为空');flag=false;return false;}
		return flag;
	}
