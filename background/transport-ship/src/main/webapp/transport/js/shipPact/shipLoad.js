...
//	startDate();
					...
					if (arr != null && arr != "" && arr.length>0){
						for (var i = 0; i<arr.length ; i++){
							...
							if (i == 0){
								for (var j = 0 ; j<arrs.length ;j++){
									$('.agree').each(function(){
										if ($(this).val() == arrs[j]){ $(this).attr('checked','checked'); }
									});
								}
						...
						for (var i = 0; i < loadAgreeMap.length; i++) {
								...
								if (loadAgreeMap[i].loadPort == loadPort){
									$(this).find('.agree').each(function(){
										if ($(this).val() == loadAgreeMap[i].agreementUuid){
											$(this).attr('checked','checked'); 
										}
									});
								}
	...
	function check(obj){
		var flag= true;
		if (obj.loadPort == null || obj.loadPort == ''){ message('请输入装港信息');flag=false; return false ;};
		if (obj.oilType == null || obj.oilType == '' ){ message($.i18n("JAVASCRIPT006"));flag=false; return false ;};
	...
	//导出数据
		...
		$('.export').on('click',function(){
		 ...
		 $('body').on('focus', '.datess, .dates', function() {
			 ...
			 //console.log(this)
		      if ($(this).parents('form').children().eq(2).find('input[name="eta"]').val() != '') {
		        if ($(this).val() != '') {	
		          ...
		          laydate.render({
		            ...
		            format: 'yyyy-MM-dd HH:mm',
		            type: 'datetime',
		            value: $(this).parents('form').children().eq(2).find('input[name="eta"]').val(),
		            closeStop: this
		          });
		        }
		      } else{
