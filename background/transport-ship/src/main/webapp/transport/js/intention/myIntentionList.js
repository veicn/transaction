layui.use(['layer', 'form', 'jquery','laypage', 'table', 'laydate','own'], function(){
	...
	$('.lookBtn').on('click',function(){	//点击详情，连续返回，须带fromPage
		...
		if(status=='1' || status=='2'){
			window.location.href=shipServer+"/intention/intentionDetail.htm?uuid="+uuid+"&status="+status+"&roleType=1&from=/intention/myIntentionList.htm&fromPage="+fromPage;
		}else if(status=='4'){
			window.location.href=shipServer+"/intention/intentionDetail3.htm?uuid="+uuid+"&roleType=1&from=/intention/myIntentionList.htm&fromPage="+fromPage;
		}
	});
	...
	$('.returnBtn').on('click',function(){
		window.location.href=shipServer+fromPage+"?status=2";
	});
