			...
			success: function(data) {  
					if(data.status == 0){
						layer.alert($.i18n("JAVASCRIPT0096"),{title:$.i18n("001"),btn:$.i18n("002")}, function(index) {
						    layer.close(index);
						    window.location.href="/shipOwnerSysShip/sysShipList.htm";
						});
					}else{
						layer.alert($.i18n("JAVASCRIPT00236"), {title:$.i18n("001"),btn:$.i18n("002")},function(index) {
							layer.close(index);
						});
					}
			},
...
function initMmsi(imo) {
	...
	params.imo= imo;
 	$.ajax({
		...
		error: function(data) {
			layer.alert($.i18n("ITSH552"),{title:$.i18n("001"),btn:$.i18n("002")}, function(index) {
				layer.close(index);
			});
        }
 	});
