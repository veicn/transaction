layui.use(['layer', 'form', 'jquery','laypage', 'table', 'laydate'], function(){
	...
	(function(){
		...
		$.ajax({
			type: "POST",
			url: shipServer+"/shipperstatements/shipperfindPactStatementsByUuid.json",
			data:{uuid:data.uuid},
			dataType: "json",
				success: function(data) {
					console.log(data)
					$('.receiptCode').html(data.datas.receiptCode);
					$('.pactCode').html(data.datas.pactCode);
					$('.payee').html(data.datas.payee);
					$('.payer').html(data.datas.payer);
