layui.use(['layer', 'form', 'jquery','laypage', 'upload','table', 'laydate','own'], function(){
	...
	$('#unload').on('click',function(){
		window.location.href=shipServer+"/shipPactNew/goodsUnload.htm?uuid="+uuid;
	});
	//返回
						...
						for (var i = 0; i < arr.length; i++) {
							...
							if (i == 0){
								...
								if (f =='0'){
									...
									if (dd.blDate != null){$('.blDate').html(new Date(dd.blDate).Format('yyyy-MM-dd'));}
									$('.blNightstool').html(formatNumber(dd.blNightstool,6,1));
									$('.blHairBarrel').html(formatNumber(dd.blHairBarrel,6,1));
									$('.blNetTonnage').html(formatNumber(dd.blNetTonnage,6,1));
									$('.blHairTonnage').html(formatNumber(dd.blHairTonnage,6,1));
									...
									$('.shHairBarrel').html(formatNumber(dd.shHairBarrel,6,1));
									$('.shHairTonnage').html(formatNumber(dd.shHairTonnage,6,1));
									$('.ratioBarrel').html(formatNumber(dd.ratioBarrel,6,1));
									$('.ratioTonnage').html(formatNumber(dd.ratioTonnage,6,1));
//								$('.shNetBarrel').html(formatNumber(dd.shNetBarrel,2,1));
//								$('.shNetTonnage').html(formatNumber(dd.shNetTonnage,3,1));
									$('.quantity').html(formatNumber(dd.quantity,6,1));
								...
								cloneDiv.find('.loadPort').html(dd.loadPort);
								...
								if (f == '0'){
									...
									//赋值
									if (dd.blDate != null){cloneDiv.find('.blDate').html(new Date(dd.blDate).Format('yyyy-MM-dd'));}
									cloneDiv.find('.blNightstool').html(formatNumber(dd.blNightstool,6,1));
									cloneDiv.find('.blHairBarrel').html(formatNumber(dd.blHairBarrel,6,1));
									cloneDiv.find('.blNetTonnage').html(formatNumber(dd.blNetTonnage,6,1));
									cloneDiv.find('.blHairTonnage').html(formatNumber(dd.blHairTonnage,6,1));
									...
									cloneDiv.find('.shHairBarrel').html(formatNumber(dd.shHairBarrel,6,1));
									cloneDiv.find('.shHairTonnage').html(formatNumber(dd.shHairTonnage,6,1));
//								cloneDiv.find('.shNetTonnage').html(formatNumber(dd.shNetTonnage,3,1));
//								cloneDiv.find('.shNetBarrel').html(formatNumber(dd.shNetBarrel,2,1));
									cloneDiv.find('.ratioBarrel').html(formatNumber(dd.ratioBarrel,6,1));
									cloneDiv.find('.ratioTonnage').html(formatNumber(dd.ratioTonnage,6,1));
									cloneDiv.find('.quantity').html(formatNumber(dd.quantity,6,1));
						...
						layer.alert($.i18n("JAVASCRIPT00103"), {title:$.i18n("001"),btn:$.i18n("002")}, {title:$.i18n("001"),btn:$.i18n("002")}, function(index) { layer.close(index);
						//window.location.href=shipServer+"/shipPact/shipPactList.htm";
						window.close();
					});
					}
...
/*	function checkObj(obj){
		var flag = true;
		if ( obj.blDate == null || obj.blDate == ''){message($.i18n("请选择提单日")) ; flag =false;return false;}
		if ( obj.blNightstool == null || obj.blNightstool == ''){message($.i18n("请输入提单")) ; flag =false;return false;}
