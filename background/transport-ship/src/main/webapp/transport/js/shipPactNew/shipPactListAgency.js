layui.use(['layer', 'form', 'laypage', 'table', 'laydate'], function(){
	...
	$('.queryBtn').on('click',function(){
		...
		window.location.href=shipServer+"/shipPact/shipPactDetailQuery.htm?uuid="+uuid;
	});
	//选择代理协议
	$('.selectBtn').on('click',function(){
		var uuid = $(this).attr('attr-data');
		window.location.href=shipServer+"/agreement/agreementMatchList.htm?uuid="+uuid;
	});
	//船务信息录入
	$('.edit').on('click',function(){
		var uuid = $(this).attr('attr-data');
		window.open(shipServer+"/shipPactNew/shipLoad.htm?shipPactUuid="+uuid+"&type=1");
	});
	//货物信息
	...
	$('.cancel').on('click',function(){
		...
		obj.uuid = uuid;
		 layer.confirm($.i18n("JAVASCRIPT0022"), {title:$.i18n("001"),btn:[$.i18n("002"),$.i18n("003")]},function(index){
				$.ajax({
					type: "POST",
					url: shipServer+"/waybill/cancelWaybill.json",
