layui.use(['layer', 'form', 'laypage', 'table', 'laydate'], function(){
	...
	$('.queryBtn').on('click',function(){
		var uuid = $(this).attr('attr-data');
		window.location.href=shipServer+"/shipPact/shipPactDetailQuery.htm?uuid="+uuid+"&fromPage=/shipPactNew/shipPactListNew.htm";
	});
	//录入结算单
	$('.jiesuan').on('click',function(){
		var uuid = $(this).attr('attr-data');
		var uuids = $(this).attr('attr-datas');
		var code = $(this).attr('attr-code');
		window.location.href=shipServer+"/statements/statementsSave.htm?shipPactUuid="+uuids+"&agreementUuid="+uuid+"&code="+code;
	});
	//查看结算单
	$('.chakan').on('click',function(){
		var uuid = $(this).attr('attr-data');
		window.location.href=shipServer+"/statements/toagreementsStatementsByUuid.htm?uuid="+uuid+"&type=1";
	});
	//船务信息录入
	$('.shipEditBtn').on('click',function(){
		var uuid = $(this).attr('attr-data');
		//window.location.href="/shipPact/shipLogistics.htm?shipPactUuid="+uuid;
		window.open(shipServer+"/shipPactNew/shipLogistics.htm?shipPactUuid="+uuid);
	});
	//货物信息
	...
	$('body').on('click','.gengduoFn',function(){
		...
		var val  = $('#'+myid).css('display');
		if (val =='none'){
			$(this).removeClass('s-tab-sq');
			$(this).addClass('s-tab-zk');
			$('#'+myid).show();
		} else {
			$(this).removeClass('s-tab-zk');
			$(this).addClass('s-tab-sq');
			$('#'+myid).hide();
		}
						...
						if(data.status == 0){
							if (data.datas.flag){
								cancelPact(uuid); 
								 ...
								 layer.confirm($.i18n(data.datas.mess), {title:$.i18n("001"),btn:[$.i18n("002"),$.i18n("003")]}, function(indexs){
									 cancelPact(uuid); 
								 })
							}
						} else {
