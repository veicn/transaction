...
public class ProductOilController {
	...
	Log log = LogFactory.getLog(ProductOilController.class);
	...
	@RequestMapping(value = "/om/productOil/ProductOilAudit.htm")
	public void audit(@RequestParam(value = "page", required = false, defaultValue = "1") int page,
			@RequestParam(value = "apage", required = false) Integer pageAgain, ModelMap modelMap, AuditQuery query) {
		...
		map.put("infoType", query.getType());
		// 取得资讯列表
		...
		mapn.put("channelMDesc", query.getType());
	...
	@RightAccess(1046)
	@RolesAccess({"admin","info_oper"})
	@RequestMapping(value = "/om/productOil/ProductOilPublish.htm")
