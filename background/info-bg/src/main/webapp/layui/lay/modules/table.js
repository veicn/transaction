layui.define(['laytpl', 'laypage', 'layer', 'form'], function(exports){
  ...
  Class.prototype.pullData = function(curr, loadIndex){
    ...
    if(options.url){ //Ajax请求
      ...
      params[request.pageSizeName] = options.pageSize;
      $.ajax({
        type: options.method || 'get'
        ,url: options.url
        ,data:JSON.stringify($.extend(params, options.where))
        ,dataType: 'json'
        ,headers: {
		      'Content-Type':'application/json;text/plain;charset=UTF-8'
		  }
        ,success: function(res){
          if(res[response.statusName] != response.status){
            that.renderForm();
            return that.layMain.html('<div class="'+ NONE +'">'+ (res[response.msgName] || '返回的数据状态异常') +'</div>');
          }
          that.renderData(res, curr, res.total), sort();
  ...
  Class.prototype.renderData = function(res, curr, count, sort){
    ...
    ,render = function(){ //后续性能提升的重点
      ...
      layui.each(data, function(i1, item1){
        ...
        ,numbers = i1 + options.pageSize*(curr - 1) + 1; //序号
    ...
    if(options.page){
      options.page = $.extend({
        ...
        ,groups: 3
        ,layout: ['prev', 'page', 'next', 'skip', 'count', 'pageSize']
