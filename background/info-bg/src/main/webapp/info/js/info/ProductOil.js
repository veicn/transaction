...
layui.use(['layer', 'form','jquery','upload','laydate'], function(){
  ...
  channel('#channelM',infoServer+'/channelM/selectListChannelM.json');
  $('#channelM').append('<option value="">请选择</option>');
  form.render('select');
  
  //channel('#channel',infoServer+'/channelSub/selectListChannelSub.json?pid='+$("#channelM option:selected").val());
  //加载父子栏目
  function channel(id,url){
	  if(url && id){
		  if(url.indexOf('channelM') != -1){
