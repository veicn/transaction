layui.use(['layer', 'table','form'], function(){
	...
	var tableOptions = {
		id:'listReload',
		url: orderServer+'/interface_list/queryLogging.json', //请求地址
		method:'POST',
		page: true
	};	
	// 初始化
	...
	$('#btn').on('click', function(){
		table.reload('listReload', {
			...
			,where: {
				inputPara: $('#inputPara').val().trim(),
				outputPara: $('#outputPara').val().trim()
	...
	table.on('tool(demo)', function(obj){
		...
		if(obj.event === 'edit'){
			layer.confirm($.i18n("orderexecute.code.00132"), {icon: 3, title:$.i18n("orderexecute.code.00133"),btn:[$.i18n("orderexecute.code.00197"),$.i18n("orderexecute.code.00198")]}, function(index){
				...
				$.ajax({
					type: "POST", 
					url:orderServer+'/interface/interfaceOutputEdit.json?inputId='+ oldData.inputId,
					beforeSend: function(){ 
					...
					success:function(data){
						if (data.status == 0) {
							// 重新载入列表
							tabReload(1);
							layer.alert($.i18n(data.message,data.params),{title:$.i18n("orderexecute.code.00148"),btn:$.i18n("orderexecute.code.00197")}, function(index) {
								// 关闭弹框
								layer.closeAll();
							});
						} else {
							tabReload(1);
							layer.close(index);
							layer.alert("error:" + $.i18n(data.message,data.params),{title:$.i18n("orderexecute.code.00148"),btn:$.i18n("orderexecute.code.00197")}, function(index) {
								table.reload();
								// 关闭弹框
								layer.closeAll();
							});
